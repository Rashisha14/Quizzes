    ...
    <main className="w-full lg:w-2/3 flex flex-col">
        ...
        <div className="flex justify-between w-full mt-6">
            <button
                ...
                disabled={currentQ === 0}
                className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 disabled:opacity-40 px-6 py-3 rounded-lg transition font-medium shadow-md"
            >
                <ChevronLeft size={20} /> Previous
            </button>
            <button
                onClick={() => setCurrentQ((prev) => Math.min(prev + 1, totalQ - 1))}
                disabled={currentQ === totalQ - 1}
                className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 px-6 py-3 rounded-lg transition font-medium shadow-md"
            >
    ...
    return (
      <div className="relative w-48 h-48 flex items-center justify-center">
        ...
        <div className="absolute flex flex-col items-center">
          ...
          <span className="text-slate-400">out of {total}</span>
        </div>
      </div>
    );
};
...
const ResultSummary = ({ result, quiz }) => {
    // FIX APPLIED HERE: Using result.totalCorrect and ensuring values are numbers
    const totalCorrect = parseInt(result.totalCorrect, 10) || 0;
    const totalAttended = parseInt(result.totalAttended, 10) || 0;
...
const ReviewQuestions = ({ questions, responses }) => {
  ...
  return (
    <div className="w-full lg:w-2/3 bg-slate-800/50 p-6 rounded-2xl border border-slate-700 flex flex-col">
      <h3 className="text-2xl font-bold text-white mb-4">Review Answers</h3>
      <div className="flex-grow p-4 rounded-lg bg-slate-900/70">
...
function QuizDetails() {
  ...
  }, [responses, quiz, timeLeft, id, token, navigate]);

  // Quiz countdown timer
  useInterval(() => {
    if (timeLeft > 0 && !result) setTimeLeft((prev) => prev - 1);
    else if (timeLeft === 0 && !result) handleSubmit();
  }, 1000);
  ...
  if (result) {
    return (
        <div className="min-h-screen w-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 to-slate-900 text-white p-4 sm:p-6 lg:p-8 relative">
            <GlowingBlobs />
